...
<%
	...
	if(name.equals("null")){ 
		...
		String exec_thread2 = "select infosThread1.g_id,user.u_id,u_name,thread.t_id,thread.t_title,remarkNums,t_time,r_time from infosThread1 right join thread on infosThread1.t_id=thread.t_id inner join user on infosThread1.u_id=user.u_id inner join remarks on thread.t_id=remarks.t_id inner join remark on remarks.r_id=remark.r_id where user.u_state_thread='1' and user.u_state_user='1'";
